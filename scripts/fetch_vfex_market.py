#!/usr/bin/env python3
"""Pull the Victoria Falls Stock Exchange (VFEX) end-of-day Market Activity
and write data/vfex-market-activity.json.

VFEX's own site (vfex.exchange) is a static corporate page with no market
data. VFEX is operated by the same group as the ZSE, though, and shares its
sibling zse.co.zw's public (no-auth) JSON API — same CloudFront host, just
exchange=VFEX instead of exchange=ZSE (confirmed by inspecting the network
calls zse.co.zw's own listed-securities page makes):

  {BASE}/api/fetch/dtr?exchange=VFEX        Daily Trade Report: turnover,
                                              trades, volume, total market
                                              cap, movers/shakers
  {BASE}/api/fetch/indices?exchange=VFEX    VFEX ALLSHARE (close + % change)
  {BASE}/api/fetch/market-capitalisation    per-company (name -> symbol map)
  {BASE}/api/notices?exchange=VFEX          VFEX notices + press releases

Unlike the ZSE, there is no public source for a full per-company daily price
table (african-markets.com, which the ZSE ticker relies on, has no VFEX
listings) -- only the movers/shakers subset the DTR itself reports. So
"gainers"/"losers" here is the whole per-company picture we can offer, not a
curated top-N of a larger set.

VFEX is USD-denominated. End-of-day only; publishes same window as the ZSE.
The CloudFront host can change if the ZSE/VFEX group redeploys — override
with VFEX_API_BASE.
"""
import json
import os
...
API_BASE = (os.environ.get("VFEX_API_BASE") or "https://ds88jcmqc11je.cloudfront.net").rstrip("/")
UA = ("Mozilla/5.0 (Macintosh; Intel Mac OS X 13_5) AppleWebKit/605.1.15 "
      "(KHTML, like Gecko) Version/16.6 Safari/605.1.15")

ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
OUT = os.path.join(ROOT, "data", "vfex-market-activity.json")
HIST = os.path.join(ROOT, "data", "vfex-history.json")


def _get(path, retries=3, backoff=3):
    """Fetch + parse JSON, retrying on transient failures — the shared
    CloudFront endpoint is flaky during the afternoon publish window (the
    exact time this job runs), and a single un-retried hiccup leaves the
    site stale until the next scheduled run."""
    url = f"{API_BASE}{path}"
    last = None
...
def _symbol_map(mcap_rows):
    """UPPERCASE company name -> ticker symbol (e.g. 'PADENGA.VX')."""
    m = {}
    for r in mcap_rows:
...
def build():
    dtr_raw = _data(_get("/api/fetch/dtr?exchange=VFEX"))
    indices_raw = _data(_get("/api/fetch/indices?exchange=VFEX"))
    try:
        mcap_raw = _data(_get("/api/fetch/market-capitalisation?exchange=VFEX"))
    except Exception as e:
        print(f"  market-cap fetch failed (symbols will be blank): {e}")
        mcap_raw = []
    try:
        notices_raw = _data(_get("/api/notices?exchange=VFEX"))
    except Exception as e:
    ...
    payload = {
        ...
        "as_of": as_of,
        "source": "Victoria Falls Stock Exchange (public market-data API)",
...
def append_history(payload):
    """Upsert the day's closes into data/vfex-history.json (one entry per
    trading day). There is no public VFEX history endpoint, so the series is
    accumulated forward from each end-of-day snapshot."""
    d = payload.get("as_of")
    if not d:
        return
    hist = {"updated": "", "series": []}
...
def main():
    try:
        ...
        print(f"ERROR fetching VFEX market data: {e}")
        sys.exit(1)

    act = payload["activity"]
    if not act.get("turnover") and not payload["indices"]:
        print("ERROR: empty VFEX payload, refusing to overwrite")
        sys.exit(1)
