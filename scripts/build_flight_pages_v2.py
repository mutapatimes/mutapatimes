...
AIRPORTS = {
    "harare-airport": {
        ...
        "stand": ("Harare International is Zimbabwe's primary international gateway. Five major long-haul carriers (Emirates, Qatar, Ethiopian, "
                  "Kenya Airways, SAA) operate daily, plus regional service to Johannesburg, Cape Town, Lusaka and Dar es Salaam."),
        "iata": "HRE", "icao": "FVHA",
        ...
        "faqs": [
            ...
            ("Can I check flight arrivals at Harare airport?",
             "Real-time arrivals and departures are not published by the Airports Company of Zimbabwe in machine-readable form. For live flight status, check the operating airline directly or use FlightAware: <a href=\"https://flightaware.com/live/airport/FVHA\" rel=\"noopener\" target=\"_blank\">flightaware.com/live/airport/FVHA</a>."),
    ...
    "bulawayo-airport": {
        ...
        "stand": ("Bulawayo's airport is Zimbabwe's second international gateway. Daily regional service to Johannesburg (Airlink, SAA, FastJet) and "
                  "domestic connections to Harare and Victoria Falls; no long-haul direct service."),
        "iata": "BUQ", "icao": "FVBU",
...
def render_airport(slug, c):
    ...
    pullout_html = f'<p class="fl-pullout">{html.escape(c["pullout"])}</p>' if c.get("pullout") else ""
    live_link = "https://flightaware.com/live/airport/" + c["icao"]
...
{facts_html}
  ...
  <aside class="fl-cta">
    <div class="fl-cta-inner">
      <h3>Live arrivals &amp; departures</h3>
      <p>Live flight status for {c["iata"]} is not published by the Airports Company of Zimbabwe in machine-readable form. For real-time flight status (gate, delay, on-time), check FlightAware:</p>
      <a class="fl-cta-btn" href="{live_link}" rel="noopener" target="_blank">FlightAware live tracker — {c["iata"]} &rarr;</a>
    </div>
  </aside>
...
{related_html}
  ...
  <section class="fl-sources" aria-label="Sources">
    ...
    <ul>
      ...
      <li><a href="{live_link}" rel="noopener" target="_blank">FlightAware live tracker for {c["iata"]}</a>.</li>
